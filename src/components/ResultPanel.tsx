...
export function PrimaryPlan({
  ...
  return (
    <div className="flex flex-col gap-3">
      <GroupCard group={group} primary weaponById={weaponById} />
      {uncovered.length > 0 && (
...
export function AlternativePlans({ groups }: { groups: PlanGroup[] }) {
  const weaponById = useWeaponById();

  if (groups.length === 0) {
    return (
      <div className="border border-dashed border-line bg-[var(--bg-1)]/40 px-4 py-6 text-center font-mono-tech text-[11px] tracking-[0.16em] text-[var(--mute)]">
        // 当前推荐已是唯一可行方案 //
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {primary ? "PRIMARY" : `ALT-${String(rank ?? "").padStart(2, "0")}`}
        </span>
